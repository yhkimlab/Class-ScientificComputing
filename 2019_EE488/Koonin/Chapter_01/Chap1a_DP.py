...
class FDM:
    def three_points_Sym(X, H):
        FPRIME = (np.float64(math.sin(X+H)) - np.float64(math.sin(X-H))) / np.float64(2*H)
        return FPRIME

    def two_points_FW(X, H):
        FPRIME = (np.float64(math.sin(X+H)) - np.float64(math.sin(X))) / np.float64(H)
        return FPRIME

    def two_points_BW(X, H):
        FPRIME = (np.float64(math.sin(X)) - np.float64(math.sin(X-H))) / np.float64(H)
        return FPRIME

    def five_points_Sym(X, H):
        FPRIME = (np.float64(math.sin(X-2*H)) - 8*np.float64(math.sin(X-H)) + 8*np.float64(math.sin(X+H)) - np.float64(math.sin(X+2*H))) / (np.float64(12*H))
        return FPRIME
