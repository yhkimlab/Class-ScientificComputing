...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
                          !display header screen, setup parameters      
      CALL INIT 
                          !main loop/ execute once for each set of param
      !5 CONTINUE 
                          !get input from screen                        
        CALL PARAM 
                          !calculate time evolution of a wavepacket     
        CALL ARCHON 
      !GOTO 5 
      END                                           
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
! Local variables:                                                      
                                 !wavepacket                            
      COMPLEX PHI(0:MAXLAT) 
                                 !wavepacket squared                    
      REAL PHI2(0:MAXLAT) 
                                 !time                                  
!      INTEGER ET       ! number of  of Evolution cycles

      REAL TIME 
                                 !normalization of the wavepacket       
      REAL TPROB,LPROB,RPROB 
                                 !average position                      
      REAL LX,RX,TX 
                                 !energy                                
      REAL E 
      ...
      INTEGER IT 
                                 !scaled time step                      
      REAL DTSCAL 
                                 !min,max reasonable size for DTSCAL    
      REAL DTMIN,DTMAX 
                                 !number of lines printed to terminal   
      INTEGER NLINES 
                                 !send to terminal                      
      INTEGER SCREEN 
                                 !make a hardcopy                       
      ...
      LOGICAL MORE,NEWDT 
                                 !which frame of the movie is it?       
      INTEGER FRAME 
                                 !graphing frequency (movies only)      
      INTEGER NFREQ 
                                 !temp variables for movies             
      REAL DIFF 

      INTEGER EL                !  Evolution loop 
! Functions:                                                            
                                 !get floating point number from screen 
      REAL GETFLT 
                                 !get yes/no answer from screen         
      INTEGER YESNO 
                                 !change from 1,0 to true and false     
      LOGICAL LOGCVT 
                                 !get integer data from screen          
      INTEGER GETINT 
...
!                                                                       
                                !setup initial PHI array                
      CALL INTPHI(PHI,PHI2) 
                                !initialize time                        
      TIME=0. 
!                                                                       
                                !def num of time steps until next prompt
      !NSTEP=40 
                                !!def graphing freq (movies only)        
      NFREQ=10 
                                !!def scaled time step                   
      !DTSCAL=.2 
...
!                                                                       
        ...
        END IF 
!                                                                       
                                 !calculate GAMMA for this DT           
        CALL TRDIAG(DT) 
        IF (TFILE) CALL TITLES(TUNIT,NLINES,DT) 
!                                                                       
                                 !loop over sets of NSTEP time steps    
!15   CONTINUE 
...
! Local variables:                                                      
                                       !lattice index                   
      INTEGER IX 
                                       !terms in matrix inversion       
      COMPLEX AZERO,CONST1 
                                       !useful constant                 
      REAL CONST2 
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      CONST1=-2.+2*SQRTM1*DX**2/DT 
      CONST2=DX*DX*K0*K0 
                                       !initial conditions for GAMMA    
      GAMMA(NPTS)=0. 
                                       !backward recursion              
      DO 20 IX=NPTS-1,0,-1 
                                       !use GAMMA(IX)=ALPHA(IX-1)       
         AZERO=CONST1-CONST2*V(IX) 
...
! Input variable:                                                       
      ...
      REAL PHI2(0:MAXLAT) 
! Output variables:                                                     
                                !left, right, and total probability     
      REAL LPROB,RPROB,TPROB 
                                !left, right, and total average position
      REAL LX,RX,TX 
! Local variables:                                                      
                                !lattice index                          
      INTEGER IX 
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      LPROB=0. 
      LX=0. 
                                         !integrate with trapezoidal rul
      DO 10 IX=1,IMID-1 
         LPROB=LPROB+PHI2(IX) 
         LX=LX+X(IX)*PHI2(IX) 
   10 END DO 
                                         !middle point is shared between
      LPROB=LPROB+PHI2(IMID)/2 
                                         !  left and right              
      LX=LX+XMID*PHI2(IMID)/2 
!                                                                       
                                         !middle point contribution     
      RPROB=PHI2(IMID)/2 
      RX=XMID*PHI2(IMID)/2 
                                         !integrate with trapezoidal rul
      DO 20 IX=IMID+1,NPTS-1 
         RPROB=RPROB+PHI2(IX) 
         RX=RX+X(IX)*PHI2(IX) 
   20 END DO 
!                                                                       
      ...
      TPROB=LPROB+RPROB 
                                         !normalize LX,RX               
      IF (LPROB .NE. 0.) LX=LX/LPROB 
      IF (RPROB .NE. 0.) RX=RX/RPROB 
                                         !total <x> is a weighted sum   
      TX=LX*LPROB+RX*RPROB 
!                                                                       
      RETURN 
      END                                           
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
! Input/Output variables:                                               
                                 !wavepacket (input)                    
      COMPLEX PHI(0:MAXLAT) 
                                 !wavepacket squared (input)            
      REAL PHI2(0:MAXLAT) 
                                 !total scaled energy (output)          
      REAL E 
! Local variables:                                                      
                                 !lattice index                         
      INTEGER IX 
...
!                                                                       
      ...
      PHI(0)=0. 
                            ! so PHI is zero there                      
      PHI(NPTS)=0. 
!                                                                       
                                                  !normalize wavepacket 
      CALL NORMLZ(PHI2,LPROB,RPROB,NORM,LX,RX,TX) 
      SQRTN=SQRT(NORM) 
      DO 30 IX=0,NPTS 
         PHI(IX)=PHI(IX)/SQRTN 
...
! Local parameters:                                                     
      ...
      CHARACTER*80 DESCRP 
      DIMENSION DESCRP(20) 
                                   !number of lines for each description
      INTEGER NHEAD,NTEXT,NGRAPH 
                                   !small number to set width of potntls
      REAL EPS 
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
!                                                                       
                                !find constants so that at X=X0+A       
      EPS=1./20. 
                                ! or X=XO-A, V(X)=EPS*V0                
      AGAUSS=LOG(1./EPS) 
                                !(doesn't apply to parabola)            
      ASTEP=TAN(PI*(1./2.-EPS)) 
!                                                                       
...
! Global variables:                                                     
      ...
      INCLUDE 'PARAM.E7' 
! Local variables:                                                      
                              !number of cycles on lattice              
      REAL CYCLES 
                              !maximum number of cycles for given NPTS  
      REAL MAXCYC 
!     map between menu items and parameters                             
      ...
      PARAMETER (INPTS  = 38) 
! Functions:                                                            
                              !converts 1 and 0 to true and false       
      LOGICAL LOGCVT 
                              !get floating point number from screen    
      REAL GETFLT 
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
!     derivative parameters                                             
      ...
      IF ((GTERM) .AND. (.NOT. TTERM)) MOVIES=.TRUE. 
                                       !spatial step                    
      DX=(VXMAX-VXMIN)/NPTS 
                                       !nearest lattice index to XMID   
      IMID=NINT((XMID-VXMIN)/DX) 
                                       !force XMID to be a lattice point
      XMID=VXMIN+IMID*DX 
                                       !change default accordingly      
      MREALS(IXMID)=XMID 
!                                                                       
!     check CYCLES with respect to lattice size                         
                                       !max number of cycles for NPTS   
      MAXCYC=REAL(NPTS)/4. 
...
!                                                                       
      ...
      CALL POTNTL 
                                             !initialize graphing param 
      IF ((GTERM) .OR. (GHRDCP)) CALL GRFINT 
!                                                                       
      RETURN 
      END                                           
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
! Global variables:                                                     
       ...
       INCLUDE 'PARAM.E7' 
! Passed variables:                                                     
                                !unit number for output (input)         
       INTEGER MUNIT 
                                !number of lines written so far (output)
       INTEGER NLINES 
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
! Input variables:                                                      
      ...
      REAL LX,RX,TX 
                                   !energy                              
      REAL E 
                                   !number of lines printed so far (I/O)
      INTEGER NLINES 
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
! Local variables                                                       
      ...
      REAL PHIGRF(0:MAXLAT) 
                                !scaling factor for PHI2                
      REAL VSCALE 
                                !lattice index                          
      INTEGER IX 
                                !Energy, SIGMA as character data        
      CHARACTER *9 CE,CSIG 
                                  !data as characters                   
      CHARACTER*9 CPROB,CTX,CTIME 
                                        !length of strings              
      INTEGER PLEN,TXLEN,TLEN,ELEN,SLEN 
                                !true length of character data          
      INTEGER LENTRU 
                                !send to terminal                       
      INTEGER SCREEN 
                                !make a hardcopy                        
...
!                                                                       
                                               !device nomination       
          !CALL GTDEV(DEVICE) 
           !DEVICE ='FILE'                     !change to graphics mode 
          !IF (DEVICE .EQ. SCREEN) CALL GMODE 
                                               !draw axes               
          !CALL LNLNAX 
      !END IF 
!                                                                       
...
!                                                                       
      ...
      !CALL XYPLOT (X,PHIGRF) 
!                                                                       
                                            !clear this graphics page   
      !IF (IPLOT .EQ. 4) CALL GPAGE(DEVICE) 
!                                                                       
      RETURN 
      END                                           
